"""
将预处理数据转换为 GraphDTA 格式

生成文件（保存到 data/{dataset}/）:
...
冷启动划分策略:
  同一蛋白序列可能出现在多个 split 中（如 CHEMBL2292 同时在 train_set 和 test_set）。
...
def find_activities_file(target_dir, target_name):
    """查找靶点的活性 CSV 文件"""
    # CHEMBL 格式
    f = target_dir / f"{target_name}_processed_activities.csv"
    if f.exists():
        return f
    # Davis 格式
    f = target_dir / "activities.csv"
    if f.exists():
        return f
    return None
...
def find_sequence_file(target_dir, target_name):
    """查找靶点的序列文件"""
    # CHEMBL 格式
    f = target_dir / f"{target_name}_processed_protein_sequence.txt"
    if f.exists():
        return f
    # Davis 格式
...
def read_activities(activities_file):
    ...
    # 通用格式: standardized_smiles, paffinity
...
def process_dataset(src_dir, dataset_name, output_base):
    ...
    # 收集所有 (canonical_smiles, sequence, affinity, target_name, split)
    all_pairs = []
    n_targets = 0
    n_targets_skipped = 0
    ...
    # 构建唯一药物和蛋白列表（保持插入顺序）
    drug_set = OrderedDict()   # canonical_smiles -> drug_id
    protein_set = OrderedDict()  # sequence -> protein_id

    # 记录每个蛋白序列出现的所有 split（用于优先级判断）
    protein_splits = OrderedDict()  # sequence -> set of splits
    ...
    target_split = {}  # protein_id -> split_name
    ...
    # 保存 ligands_can.txt (JSON OrderedDict)
    ...
    # 保存 proteins.txt (JSON OrderedDict)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Convert ChEMBL or Davis to GraphDTA format")
    parser.add_argument("dataset", choices=["chembl", "davis"])
    parser.add_argument("input_dir", type=Path)
    parser.add_argument("--output-root", type=Path, default=BASE / "data")
    args = parser.parse_args()
    process_dataset(args.input_dir, args.dataset, args.output_root)
