"""批量训练 kiba 数据集的 4 个 GraphDTA 模型"""
import os, sys, json, time, subprocess, argparse
from pathlib import Path
from datetime import datetime

BASE_DIR = Path(__file__).resolve().parent
STATE_FILE = BASE_DIR / "batch_kiba_state.json"
LOG_DIR = BASE_DIR / "batch_kiba_logs"

TASKS = [
    {"name": "kiba_GINConvNet", "dataset_idx": 2, "model_idx": 0},
    {"name": "kiba_GATNet",     "dataset_idx": 2, "model_idx": 1},
    {"name": "kiba_GAT_GCN",    "dataset_idx": 2, "model_idx": 2},
    {"name": "kiba_GCNNet",     "dataset_idx": 2, "model_idx": 3},
]
...
def print_metrics_table(results):
    ...
    for r in results:
        if r.get("status") == "success" and "idx" in r:
            ...
            if metrics:
                model_name = MODEL_NAMES[task['model_idx']]
                row = {"task": f"kiba + {model_name}", "best_epoch": metrics.get("best_epoch", "-")}
    ...
    summary_file = BASE_DIR / "kiba_training_summary.json"
    with open(summary_file, "w") as f:
        json.dump({"timestamp": datetime.now().isoformat(), "results": all_metrics}, f, indent=2)
    print(f"\n汇总已保存: {summary_file}")

def main():
    parser = argparse.ArgumentParser(description="批量训练 kiba GraphDTA 模型")
    ...
    for r in results:
        if "idx" in r:
            ...
            print(f"  {icon} Task {r['idx']}: kiba + {m} [{r['status']}]")
